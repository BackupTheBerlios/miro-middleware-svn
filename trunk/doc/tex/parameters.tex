...
\item[configuration]
  ...
  \item[section] Each component of \miro can have its own section
    ...
    \item[parameter] Parameters of a component are grouped and
      ...
      Attributes: 
      \begin{itemize}
      \item \textit{name} Name of the parameter. On top level, the
        parameter name attribute of the parameter tag denotes the name
        of the parameter type. The parameter type definitions are
        described in section \ref{SEC:PARAMETER_TYPES}.
      \end{itemize}  
      ...
      \end{description}  
    \item[instance] As the name of a top level parameter tag denotes
      the type name of the parameter set, multiple instances of
      parameters sets in one section become ambiguous. So this tag
      solves the problem. It has two mandatory attributes:
      
      Attributes: 
      \begin{itemize}
      \item \textit{name} Name of the parameter. In this case, an arbitrary name.
      \item \textit{type} The type name of the parameter.
      \end{itemize}
      
      Appart from this, instance tags behave identical, to toplevel
      parameter tags. That is, the values of the parameter set
      elements are described in nested parameter tags.
    \end{description}
  \end{description}
\end{description}
...
\ref{SEC:POLICY_EDITOR} as well as the {\tt ConfigEditor} (sec.
\ref{SEC:CONFIG_EDITOR}) use the parameter description files for GUI
