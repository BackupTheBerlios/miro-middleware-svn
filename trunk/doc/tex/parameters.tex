...
Robot control programs and robotic algorithms are full of
parameters. Different robot types vary in shape and sensor
configuration, but also different robots of the same type tend to vary
slightly. For instance they some of them are
equipped with further sensory or actuatory devices not present in the
standard configuration. Also damage and repair of parts of a robot
during its lifetime result in an individual robot, which has its own
...
general conecept of parameter sets and configuration files is
scetched. Then the
...
the then following section the description and implementation of
parameter sets and the the mapping to the actual parameter files will
...
Additionally there can also be an absolute path be specified.
...
be initialized by parsing the configuration file. To fasiliate the
...
configuration of the services or \miro but also by other tools within
...
based editing of the behaviour parameters. To fasciliate the reuse of
...
See {\tt \$MIRO\_ROOT/examples/params/Parmeters.xml}
for a small initial example.
...
See {\tt \$MIRO\_ROOT/examples/params/Parmeters.h}
for a small initial example.
