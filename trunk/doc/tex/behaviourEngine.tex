...
The {\tt MotionArbiter} uses the very a simple fixed priority, winner
...
\item Load Behaviour-Description-File \\
  ...
  has to contain a valid behaviour description. - Verifying, whether
  the load was successful is currently only possible, by querying the
  behaviour description file name via the menu item above. On the
  positive side the currently loaded description is memorized in the
  file .PolicyEditorConfig.xml file in the users home directory for
  further runs of the policy editor.
\end{itemize}
\subsubsection{Help}
...
\item Rename transition \\
  ...
  as well as the target of the transition. - Be carful, the new entry
...
\item Set parameters \\
  ...
  represent. - Please only edit the value field. Also, be carful, the
...
\begin{description}
  ...
  \item[config\_group]
    ...
    \item \textit{name} The name of the config group. Within the
      ...
      specified type are allowed to occur.
    \end{itemize}
