...
\item[Odometry:]
  Exceptions first:
  There does not exist a dedicated odometry service. This interface is
  the base class of the motion service (see next item). It encapsulates
  the odometry (dead reckoning) sensor, where as the motion interface
  contains methods for controlling actuatory side of the motors.
\item[Motion:]
  The motion service registers the {\tt Miro::Motion} interface as
  {\tt Motion}. It inherits the Odometry interface and enriches it
  witch general movement commands.
\item[RangeSensor:]
  The range sensor interface is the general abstraction of all range
  sensor devices, such as sonar, laser range finders etc. It offers a
  method for querying the sensors physical configuration as well as
  the latest sensor reading. 
  %% See the {\tt qtRangeSensor} program in the demo
  %% directory for an example on how to use this information. 
  The range sensor interface becomes
  registered at the naming service under the name of the actual
  sensory device.
  \begin{description}
  \item[Sonar:] The sonar service registers the {\tt Miro::Sonar}
    interface as {\tt Sonar}.  It provides an interface to the very
    common ultrasonic sensors for robots. The interface is derived from
    the {\tt Miro::RangeSensor} interface.
  \item[Infrared:] It registers the {\tt Miro::Infrared} interface as
    {\tt Infrared}. As a range sensor, it is derived
    from the {\tt Miro::RangeSensor}, too.  Since this service is
    wildly untested at the moment, there does not exist a generalized
    message data format at the moment.
  \item[Tactile:] Some robots have bumpers as some sort of ''it's too
    ...
    just moving on. The {\tt Miro::Tactile} interface is registered
    as {\tt Tactile}.  Since it can be interpreted as a (very
    limited) range sensor, it is also derived from the {\tt Miro::RangeSensor}.
  \item[Laser:] The laser scanner service registers the
    {\tt Miro::Laser} as {\tt Laser}. It provides very accurate
    180° distance measurements, with up to 48Hz. As a range sensor, it
    is derived from the {\tt Miro::RangeSensor}, too.
  \end{description}
\item[Stall:]
...
\item[Video:] The video service registers the {\tt Miro::Video}
  ...
  excessive overhead, the video service also enables access
...
\begin{description}
\item[Odometry:]
  The odometry servant and all derived services generates periodical
  {\tt Miro::MotionStatusIDL} events (normally at 10Hz), that
...
\item[RangeSensor:] The range sensor servant and all derived services
  ...
  \end{description}
  The event type name is that of the actual sensor: {\tt Sonar},
  {\tt Laser}, {\tt Tactile}, etc.
...
\item[Buttons:]
  They also provide the event triggered
  communications model that uses the event type {\tt Button}. For
  the event data see the individual robots descriptions.
\end{description}
...
\item[b21Base:] This is a collection of the robots main services. This
  ...
  \item {\tt Miro::Sonar} \\
    ...
    as {\tt Sonar}. The specializations of this interface are {\em
      deprecated}.
  \item {\tt Miro::Infrared} \\
    The derived {\tt Miro::RangeSensor} interface registers itself
    as {\tt Infrared}. The specializations of this interface are {\em
      deprecated}.
  \item {\tt Miro::Tactile} \\
    The derived {\tt Miro::RangeSensor} interface registers itself
    as {\tt Tactile}. The specializations of this interface are {\em
      deprecated}.
  \item {\tt Miro::B21Buttons} \\
    The B21 has four buttons, that can be used as a simple
    user interface. The {\tt Miro::B21Buttons} interface is
    supported by this service. It sends
    {\tt Miro::B21ButtonStatusIDL} events, if a button is pressed
    or released.
  \end{itemize}

\item[sickLaserService:]
  This is the service to access the SICK laser scanner. It supports
  the {\tt Miro::Laser} interface.
\item[dtlkSpeech:]
...
in our laboratory is as follows: Start b21Base, laserService and
b21Buttons on the left computer, imageService and dpPantilt and
...
\item[sparrowBase:]
  ...
  \item {\tt Miro::Infrared} \\
    ...
    as {\tt Infrared}.
  \item {\tt Miro::PanTilt} \\
    The standard {\tt Miro::Pan} interface is registered as
    {\tt PanTilt}.
  \end{itemize}

  Note that the
  stall service sends {\tt Miro::SparrowStallIDL} events and
  the buttons service {\tt Miro::SparrowButtonStatusIDL} events.
\item[videoService:]
...
\item[pioneerBase:]
  ...
  \item {\tt Miro::PioneerStall} \\
    The {\tt Miro::PioneerStall} interface (derived from {\tt Miro::Stall}) is registered as {\tt Stall}.
  \item {\tt Miro::Sonar} \\
    The {\tt Miro::RangeSensor} interface registers itself
    as {\tt Sonar}.
  \end{itemize}
...
finder of type Sick pls200 (TODO: check this?). This sensor is
connected to the controlling PC via a serial line. It is delivered
either with a RS-232 or a RS-422 compliant interface. 
...
%In the \miro framework, there are two different kinds of services:
%\begin{description}
%\item[Low-level Services] are services that belong to a hardware device
%  (e.g.\ laser scanner, camera, etc.) and perform the hardware
%  abstraction for this device. These services mostly have to be run on
%  the specific machine to which the device is attached to.
%\item[High-level Services] are services that are not linked to a
%  hardware device but provide software services. It depends on the
%  resources and transfer capacities where these services have to be
%  run.
%\end{description}

%\newpage
%\section{Low-level Services}

%The following sections show the main commands of the current services
%with a short description. For a more detailed description, see the
%class reference in Chapter \ref{SEC_CLASS_REFERENCE}.

%\subsection{BaseServer}

%\begin{verbatim}
%    // compound commands for movement

%    void limp();
%    void halt();

%    // set the speed
%    void setVelocity(in Velocity vel);
%    VelocityData getVelocity();
%    VelocityData getWaitVelocity();

%    // position estimation

%    // tell robot to see his current position as
%    // the given coordinates and heading
%    void initPosition( in Position pos );
%    void updatePosition( in Position dPos);
%    PositionData getPosition();
%    PositionData getWaitPosition();
...
%    void rotateLeft();
%    void rotateRight();
%    void rotateToPosition(in unsigned long position);
%    void rotateRelative(in long relative);
%    void rotateTorque(in long relative);
%    void rotatePower(in long relative);

%    void setRotateVelocity(in unsigned long velocity);
%    void setRotateAcceleration(in unsigned long accel);
%    void setRotateTorque(in unsigned long tork);
...
%    void translateToPosition(in unsigned long position);
%    void translateRelative(in long relative);
%    void translateTorque(in long relative);
%    void translatePower(in long relative);

%    void setTranslateVelocity(in unsigned long velocity);
%    void setTranslateAcceleration(in unsigned long accel);
%    void setTranslateTorque(in unsigned long tork);

%    // Queries

%    double getWaitRotateCurrent();
%    double getWaitTranslateCurrent();
%    double getWaitBatteryVoltage();
  
%    // ensuring to be the only one commanding the base:

%    void  requestWaitOdometryLock(in unsigned long priority);
%    void  releaseOdometryLock();
%\end{verbatim}



%\subsection{LaserServer}

%\begin{verbatim}
%    LaserScan            getScan();
%    LaserScan            getWaitScan();

%    void                 setContinousMode();
%    void                 setPollingMode();

%    LaserScanDescription getScanDescription();
%\end{verbatim}



%\subsection{SonarServer}

%\begin{verbatim}
%    SonarData getData();
%    SonarData getWaitData();

%    SonarScanDescription  getDataDescription();
%\end{verbatim}
...
%\subsection{TactileServer}

%\subsection{ButtonServer}

%The ButtonServer is not available yet.

%\subsection{ImageServer}
