...
//
// $Id: SickLaserService.cpp,v 1.8 2007-02-25 07:09:42 hutz Exp $
//
...
LaserServer::LaserServer(int argc, char *argv[]) :
			...
			ec_(::Laser::Parameters::instance()->notify ? tryResolveName<
					CosNotifyChannelAdmin::EventChannel>("EventChannel")
					: CosNotifyChannelAdmin::EventChannel::_nil()),
			structuredPushSupplier_((!CORBA::is_nil(ec_.in())) ? new StructuredPushSupplier(ec_.in(), namingContextName)
					: 
					NULL), odoTracking_(::Laser::Parameters::instance()->positionStamps ? new OdometryTracking(ec_.in(), namingContextName) : 
			NULL), laser_(odoTracking_, ::Laser::Parameters::instance()->laserDescription, structuredPushSupplier_),
			laserStatistic_(::Laser::Parameters::instance()->statistics ? new LaserStatistic() : NULL), connection_(reactorTask_.reactor(), laser_,
					laserStatistic_) {
...
void calculateLaserDescription(Laser::Parameters * parameters) {
	...
	for (int i = 0; i < values; ++i) {
		...
		parameters->laserDescription.group[0].sensor[i].alpha
				= Miro::deg2Rad(-(parameters->fov/2) + i
						* parameters->scanResolution);
