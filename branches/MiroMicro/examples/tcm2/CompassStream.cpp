...
CompassStream::CompassStream(EventChannel_ptr _ec,
  ...
  Super(_ec)
{
  ...
  added[0].type_name = CORBA::string_dup("Compass");

  setSubscriptions(added);
}

CompassStream::~CompassStream()
{
}

void
CompassStream::push_structured_event(const StructuredEvent & notification
				    ACE_ENV_ARG_DECL_NOT_USED)
  throw(CORBA::SystemException, CosEventComm::Disconnected)
{
  const Miro::CompassEventIDL * pTCM2Data;

  if (notification.remainder_of_body >>= pTCM2Data)
    cout << *pTCM2Data << endl;
  else
    cerr << "No COMP03EventIDL in message." << endl;
}
...
main(int argc, char *argv[])
{
  ...
  try {
    ...
    CompassStream pushConsumer(ec.in(), server.namingContextName);
